...
Exact string match after normalizing to uppercase and stripping non-alphanumeric characters.
...
Model numbers are matched by extracting numeric portions. A product with model "870 EVO" from Pichau and model "870" from KaBuM are matched because the number "870" appears in both.
...
penalty = (unknown_rule_count / total_rules) × 0.15
confidence = clamp(match_ratio - penalty, 0, 1)
```

Unknown rules (null values) do not penalize as severely as negative rules.
...
See `src/products/ProductMatch.js` for the full implementation.
