...
class StorageDeviceExtractor {
  ...
  _getModelAndFamily(title) {
    const patterns = [
      { regex: /(\d{3,4})\s+(EVO|PRO|PLUS|TFT|ITL|QVO)/i, familyIdx: 2, modelIdx: 1 },
      { regex: /(\d{3,4})\s+(Green|Blue|Black)\b/i, familyIdx: 2, modelIdx: 1 },
      ...
      { regex: /(\d{3,4})\b/i, familyIdx: 1, modelIdx: 1, defaultFamily: true },
    ];
    for (let { regex, familyIdx, modelIdx, defaultFamily } of patterns) {
      ...
      if (m && m[1]) {
        ...
        return { model: m[modelIdx], family: family || null, confidence: 0.85 };
      }
    }
  ...
  _detectInterface(title) {
    ...
    if (detectedInterface === 'NVMe') { protocol = 'NVMe'; protocolConf = 0.95; }
      ...
      protocol = /\bNV[Mm]e?\b/i.test(title) ? 'NVMe' : 'SATA';
      protocolConf = 0.75;
    }
    let pcieGen = null;
    let pcieConf = 0.7;
  ...
  _extractManufacturerSku(title, family) {
    ...
    const wdCodePattern = /\b(WD[SBE]\d{3,4}T\dG\d[A-Z]+)\b/i;
    skuMatch = wdCodePattern.exec(title);
    if (skuMatch) return { manufacturerSku: skuMatch[1].toUpperCase(), confidence: 0.9 };
