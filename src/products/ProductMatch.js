...
function matchProducts(left, right) {
  if (left == null || right == null) {
    return buildEvidence([
      evidenceField('manufacturerSku', false, 'One product missing', 'One or both products are null'),
      evidenceField('brand', false, 'One product missing'),
      evidenceField('model', false, 'One product missing'),
      evidenceField('family', false, 'One product missing'),
      evidenceField('capacity', false, 'One product missing'),
      evidenceField('interface', false, 'One product missing'),
      evidenceField('formFactor', false, 'One product missing'),
    ]);
  }

  const results = [];

  // Rule 1: Manufacturer SKU (highest priority)
  results.push(compareSku(left.manufacturerSku, right.manufacturerSku));

  // Rule 2: Brand
  results.push(compareBrand(left.brand, right.brand));

  // Rule 3: Model
  results.push(compareModel(left.model, right.model));

  // Rule 4: Family
  results.push(compareFamily(left.family, right.family));

  // Rule 5: Capacity
  results.push(compareCapacity(left.capacityGB, right.capacityGB));

  // Rule 6: Interface OR Protocol (whichever is present)
  if (left.interface || right.interface) {
    results.push(compareInterface(left.interface, right.interface));
  } else if (left.protocol || right.protocol) {
    results.push(compareProtocol(left.protocol, right.protocol));
  } else {
    results.push({ruleName: 'interface', matched: true, detail: false, label: 'Both unknown', leftValue: null, rightValue: null});
  }

  // Rule 7: PCIe generation
  results.push(comparePcieGeneration(left.pcieGeneration, right.pcieGeneration));

  // Rule 8: Form factor
  results.push(compareFormFactor(left.formFactor, right.formFactor));

  return buildEvidence(results);
}
